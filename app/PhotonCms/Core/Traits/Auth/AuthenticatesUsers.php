...
trait AuthenticatesUsers
{
    ...
    public function login(Request $request)
    {
        ...
        if ($throttles && $lockedOut = $this->hasTooManyLoginAttempts($request)) {
            ...
            if ($request->ajax() || $request->wantsJson()) {
                return $this->responseRepository->make('LOGIN_TOO_MANY_ATTEMPTS');
            } else {
        ...
        if (Auth::guard($this->getGuard())->attempt($credentials, $request->has('remember'))) {
            ...
            if ($request->ajax() || $request->wantsJson()) {
                ...
                return $this->responseRepository->make('USER_LOGIN_SUCCESS', ['user' => $user]);
            } else {
        ...
        // Check if it failed because user was not confirmed
        if (\Config::get('photon.use_registration_service_email')) {
    ...
    {
        $user = \Photon\PhotonCms\Core\Entities\User\User::whereConfirmationCode($confirmationCode)->first();
