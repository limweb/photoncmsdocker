...
/**
 * Compares instances of two Field-s.
 */
class FieldComparator implements InstanceComparatorInterface
{

    /**
     * Compare method for the Field entity.
     *
    ...
    {
        $input = func_get_args();
        if (!isset($input[0]) && !($input[0] instanceof Field)) {
            throw new PhotonException('NOT_INSTANCE_OF_FIELD', ['given' => get_class($input[0]), 'expected' => '\Photon\PhotonCms\Core\Entities\Field\Field']);
        }
        if (!isset($input[1]) && !($input[1] instanceof Field)) {
            throw new PhotonException('NOT_INSTANCE_OF_FIELD', ['given' => get_class($input[1]), 'expected' => '\Photon\PhotonCms\Core\Entities\Field\Field']);
        }
        ...
        foreach ($first as $key => $item) {
            if (isset($second[$key]) && $item !== $second[$key]) {
                $data[$key] = [
                    'old' => $item,
                    'new' => $second[$key]
                ];
